...
export const RandomQuiz = () => {
  ...
  const API_URL = "http://127.0.0.1:8000/api/quiz/rand/" + topic;
  const [dataState] = ConnectApi(API_URL);
  const a = dataState.data.flatMap((q) => q.answers);
  const ac = a.length;
    ...
    <React.Fragment>
      ...
      <Container component="main" maxWidth="xs">
        <div className={classes.paper}>
          {dataState.data.map(({ title, answers }, i) => (
            <div key={i}>
              <Typography component="h1" variant="h5">
                {title}
              </Typography>
              {answers.map(({ answer_text, id }) => (
                <RadioGroup>
                  <FormControlLabel
                    control={
                      <Checkbox
